...
describe("extractKey", () => {
  it("extracts from x-engram-key header", () => {
    const req = new Request("http://localhost/api/mcp", {
      headers: { "x-engram-key": TEST_KEY },
    });
  ...
  it("prefers x-engram-key header over Bearer", () => {
    const req = new Request("http://localhost/api/mcp", {
      headers: {
        "x-engram-key": "header-key",
...
describe("validateAccessKey", () => {
  beforeEach(() => {
    vi.stubEnv("ENGRAM_ACCESS_KEY", TEST_KEY);
  });
  ...
  it("throws when env var is not set", () => {
    vi.stubEnv("ENGRAM_ACCESS_KEY", "");
    expect(() => validateAccessKey(TEST_KEY)).toThrow(
      "ENGRAM_ACCESS_KEY is not configured",
    );
  });
});
