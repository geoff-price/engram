...
export function validateAccessKey(key: string): boolean {
  const expected = process.env.ENGRAM_ACCESS_KEY;
  if (!expected) throw new Error("ENGRAM_ACCESS_KEY is not configured");
...
export function extractKey(req: Request): string | null {
  // Header: x-engram-key
  const headerKey = req.headers.get("x-engram-key");
